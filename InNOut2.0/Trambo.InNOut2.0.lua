script_name="@TRAMBO: In N Out 2.0"
...
--ADDITIONAL EFFECTS
  ...
  --buttons
  buttons = {loadPr,save,manage,char,word,reset,cancel}
  buttons_save = {ok,cancel}
  buttons_manage = {remove,rename,"Preset File"}
  
  choice, res = ADD(GUI,buttons)
  
  while choice == save or choice == loadPr or choice == manage do
    if choice == save then
      local status = false
      local pass = true
...
function apply_fx(sub, sel, time, fade, dir, ydir, zdir, scale, rot, blur, alignRes , choice)
  ...
  for si, li in ipairs(sel) do
    ...
    if la1 == "&H00&" and la3 == "&H00&" and la4 == "&H00&" then
      styleAlpha = "\\alpha&H00&"
      hasAlpha_style = false
    elseif la1 ~= "&H00&" or la3 ~= "&H00&" or la4 ~= "&H00&" then
      styleAlpha = "\\alpha&H00&"
      if la1 ~= "&H00&" then styleAlpha = styleAlpha .. "\\1a" .. la1 end
      if la3 ~= "&H00&" then styleAlpha = styleAlpha .. "\\3a" .. la3 end
      if la4 ~= "&H00&" then styleAlpha = styleAlpha .. "\\4a" .. la4 end
    else 
...
function updateGUI(tin,tout,fin,fout,dirin,dirout,ydirin,ydirout,yin,yout,zdirin,zdirout,zin,zout,fscxin,xScalein,fscxout,xScaleout,fscyin,yScalein,fscyout,yScaleout,fscin,scalein,fscout,scaleout,frxin,xRotin,frxout,xRotout,fryin,yRotin,fryout,yRotout,frzin,zRotin,frzout,zRotout,blin,blurin,blout,blurout,anFlag,an,presetList,curpreset)
  local g = 
  {
    -- position labels
    { class = "label", x = 1, y = 0, width = 1, height = 1, label = "In"},
