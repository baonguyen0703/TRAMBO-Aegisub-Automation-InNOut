script_name="@TRAMBO: In N Out 1.0"
script_description="Make characters or words appear one by one"
...
--X 
none = "None"; l = "L -> R"; r = "L <- R"; lr = "-> C <-"; rl = "<- C ->"; 
--Y
tml = "T -> M , L -> R"; tm = "T -> M"; bm = "B -> M"; tbm = "T -> M <- B"; mtm = "M -> T -> M"; mbm = "M -> B -> M"; sin = "SINE WAVE"; mt = "M -> T"; mb = "M -> B"; mtb = "T <- M -> B"; rand = "Random"
--Z
...
function open_dialog(sub,sel)
  ...
  GUI = 
  {
    ...
    { class = "dropdown", x = 1, y = 3, width = 1, height = 1, items = {"None", "Random", "L -> R","L <- R","-> C <-","<- C ->"}, value = dirin, name = "inD", hint = "L = Left, R = Right, C = Center"},
    { class = "dropdown", x = 2, y = 3, width = 1, height = 1, items = {"None", "Random", "L -> R","L <- R","-> C <-","<- C ->"}, value = dirout, name = "outD", hint = "L = Left, R = Right, C = Center"},
...
--ADDITIONAL EFFECTS
    ...
    --{ class = "checkbox", x = 5, y = 8, width = 1, height = 1, value = false, label = "an", name = "align"},
    --{ class = "dropdown", x = 6, y = 8, width = 1, height = 1, items = {"1", "2", "3", "4", "5", "6", "7", "8", "9"}, value = an, name = "align_val"},
  }

  --buttons

  buttons = {char,word,cancel}
...
function apply_fx(sub, sel, time, fade, dir, ydir, zdir, scale, rot, blur, alignRes , choice)
  ...
  for si, li in ipairs(sel) do
    ...
    if choice == char or choice == word then
      ...
      if alignRes[1] then
        local orgAn = "" .. line.styleref.align; aegisub.debug.out("orgAn:" .. orgAn)
        if line.text:find("\\an%d") then
          aegisub.debug.out("\ntrue")
          orgAn = line.text:match("\\an(%d)"); aegisub.debug.out("\nOrgAn: " .. orgAn) 
          line.text = line.text:gsub("\\an%d", "\\an" .. an); aegisub.debug.out(line.text)
        else
          line.text = "{\\an" .. an .. "}" .. line.text; aegisub.debug.out(line.text)
        end

        local orgX = "" .. line.x; local orgY = "" .. line.y
    ...
    if (time[1] ~= 0 and dir[1] ~= none) or (time[2] ~= 0 and dir[2] ~= none) then
      ...
      local temptext = ""
      local add2out = ""
      if dir[1] == none then add2out = "\\alpha&H00&" end
...
--OUT EFFECTS          
        if time[2] ~= 0 and dir[2] ~= none then
          ...
          for i, v in ipairs(ctime1) do
            if dir[2] == v then ctime = math.floor(time[2] / cn) end
          end
          ...
          if dir[2] == rand then 
            ...
            for i = 1,#p,1 do
              local j = pOrder[i]
              if t[p[j]] ~= " " and t[p[j]] ~= "*" then
                n = n + 1 
                if i ~= #p then
                  t[p[j]] = "{" .. add2out .. "\\t(" .. tostring(tout + ctime*(n-1)) .. "," .. tostring(tout + ctime*n) .. "," .. "\\alpha&HFF&)}" .. t[p[j]]
                else
                  t[p[j]] = "{" .. add2out .. "\\t(" .. tostring(tout + ctime*(n-1)) .. "," .. tostring(dur) .. "," .. "\\alpha&HFF&)}" .. t[p[j]]
                end
              end
            end
            --out "L -> R"   
          elseif dir[2] == l then
            ...
            for i = 1,#p,1 do
              if t[p[i]] ~= " " and t[p[i]] ~= "*" then
                n = n + 1 
                if i ~= #p then
                  t[p[i]] = "{" .. add2out .. "\\t(" .. tostring(tout + ctime*(n-1)) .. "," .. tostring(tout + ctime*n) .. "," .. "\\alpha&HFF&)}" .. t[p[i]]
                else
                  t[p[i]] = "{" .. add2out .. "\\t(" .. tostring(tout + ctime*(n-1)) .. "," .. tostring(dur) .. "," .. "\\alpha&HFF&)}" .. t[p[i]]
                end
              end
            end
          ...
          elseif dir[2] == r then 
            ...
            for i = 1,#p,1 do
              if t[p[i]] ~= " " and t[p[i]] ~= "*" then
                n = n + 1 
                if n == cn then
                  t[p[i]] = "{" .. add2out .. "\\t(" .. tostring(tout) .. "," .. tostring(dur - ctime*(n-1)) .. "," .. "\\alpha&HFF&)}" .. t[p[i]]
                else
                  t[p[i]] = "{" .. add2out .. "\\t(" .. tostring(dur - ctime*(n)) .. "," .. tostring(dur - ctime*(n-1)) .. "," .. "\\alpha&HFF&)}" .. t[p[i]]
                end
              end
            end
            --out -> C <-   
            ...
            for i = 1,#p,1 do
              if t[p[i]] ~= " " and t[p[i]] ~= "*" then
                ...
                elseif n == mid then
                  ...
                  t[p[i]] = "{" .. add2out .. "\\t(" .. tostring(tout + ctime*(cn-n)) .. "," .. tostring(tout + ctime*(cn-n+1)) .. "," .. "\\alpha&HFF&)}" .. t[p[i]]
                end
              end
            end
            --out <- C ->  
            ...
            for i = 1,#p,1 do
              if t[p[i]] ~= " " and t[p[i]] ~= "*" then
                ...
                elseif n == mid then
                  ...
                  t[p[i]] = "{" .. add2out .. "\\t(" .. tostring(dur - ctime*(cn-n+1)) .. "," .. tostring(dur - ctime*(cn-n)) .. "," .. "\\alpha&HFF&)}" .. t[p[i]]
                end
              end
            end
          end
...
--IN EFFECTS          
        if time[1] ~= 0 and dir[1] ~= none then
          local ctime
          for i, v in ipairs(ctime1) do
            if dir[1] == v then ctime = math.floor(time[1] / cn) end
          end
          for i, v in ipairs(ctime2) do
          ...
          if dir[1] == rand then 
            ...
            for i = 1,#p,1 do
              ...
              if t[p[j]] ~= " " and t[p[j]] ~= "*" then
                n = n + 1 
                if i ~= #p then
                  t[p[j]] = "{\\alpha&HFF&\\t(" .. tostring(ctime*(n-1)) .. "," .. tostring(ctime*n) .. "," .. "\\alpha&H00&)}" .. t[p[j]]
                else
                  t[p[j]] = "{\\alpha&HFF&\\t(" .. tostring(ctime*(n-1)) .. "," .. tostring(time[1]) .. "," .. "\\alpha&H00&)}" .. t[p[j]]
                end
              end
            end
            --"L -> R"
          elseif dir[1] == l then 
            ...
            for i = 1,#p,1 do
              if t[p[i]] ~= " " and t[p[i]] ~= "*" then
                n = n + 1 
                if i ~= #p then
                  t[p[i]] = "{\\alpha&HFF&\\t(" .. tostring(ctime*(n-1)) .. "," .. tostring(ctime*n) .. "," .. "\\alpha&H00&)}" .. t[p[i]]
                else
                  t[p[i]] = "{\\alpha&HFF&\\t(" .. tostring(ctime*(n-1)) .. "," .. tostring(time[1]) .. "," .. "\\alpha&H00&)}" .. t[p[i]]
                end
              end
            end
          ...
          elseif dir[1] == r then 
            ...
            for i = 1,#p,1 do
              if t[p[i]] ~= " " and t[p[i]] ~= "*" then
                n = n + 1 
                if n == cn then
                  t[p[i]] = "{\\alpha&HFF&\\t(" .. tostring(0) .. "," .. tostring(time[1] - ctime*(n-1)) .. "," .. "\\alpha&H00&)}" .. t[p[i]]
                else
                  t[p[i]] = "{\\alpha&HFF&\\t(" .. tostring(time[1] - ctime*(n)) .. "," .. tostring(time[1] - ctime*(n-1)) .. "," .. "\\alpha&H00&)}" .. t[p[i]]
                end
              end
            end
            -- -> C <-  
            ...
            for i = 1,#p,1 do
              if t[p[i]] ~= " " and t[p[i]] ~= "*" then
                ...
                elseif n == mid then
                  ...
                  t[p[i]] = "{\\alpha&HFF&\\t(" .. tostring(ctime*(cn-n)) .. "," .. tostring(ctime*(cn-n+1)) .. "," .. "\\alpha&H00&)}" .. t[p[i]]
                end
              end
            end
            -- <- C ->  
            ...
            for i = 1,#p,1 do
              if t[p[i]] ~= " " and t[p[i]] ~= "*" then
                ...
                elseif n == mid then
                  ...
                  t[p[i]] = "{\\alpha&HFF&\\t(" .. tostring(time[1] - ctime*(cn-n+1)) .. "," .. tostring(time[1] - ctime*(cn-n)) .. "," .. "\\alpha&H00&)}" .. t[p[i]]
                end
              end
            end
          end
...
--COLOR IN
          ...
          for i = 1,#p,1 do
            if t[p[i]] ~= " " and t[p[i]] ~= "*" then
              --if time[2] == 0 or dir[2] == none then
              if bl[i] ~= "" then
                --if bl[i]:find("\\%da&") then
                for v in bl[i]:gmatch("\\%da&.-&") do 
                  orgAlpha = adjust_org_tag(orgAlpha,v)
              ...
              --end 
              t[p[i]] = add_to_t(t[p[i]],orgColor,orgAlpha)
              local temp = t[p[i]]:match("\\t.-%)")
              if temp:find("\\alpha&H00&") then 
                t[p[i]] = t[p[i]]:gsub("\\t.-%)",temp:gsub("\\%da&.-&",""),1)
              end
            end
          end          
--SCALE IN
...
function adjust_org_tag(org, new)
  local tag = {"\\c&.-&","\\1c&.-&","\\2c&.-&","\\3c&.-&","\\4c&.-&", "\\alpha%w+","\\1a&.-&","\\2a&.-&","\\3a&.-&", "\\4a&.-&", "\\fscx%d+%.?%d*", "\\fscy%d+%.?%d*","\\fsc%d+%.?%d*", "\\fs%d+%.?%d*", "\\fsp%d+%.?%d*", "\\i%d", "\\b%d+%.?%d*", "\\u%d", "\\s%d", "\\bord%d+%.?%d*", "\\xbord%d+%.?%d*", "\\ybord%d+%.?%d*", "\\shad%d+%.?%d*", "\\xshad-?%d+%.?%d*", "\\yshad-?%d+%.?%d*", "\\be%d+%.?%d*", "\\blur%d+%.?%d*", "\\fn%w", "\\fr-?%d+%.?%d*", "\\frx-?%d+%.?%d*", "\\fry-?%d+%.?%d*", "\\frz-?%d+%.?%d*", "\\fa-?%d+%.?%d*", "\\fax-?%d+%.?%d*", "\\fay-?%d+%.?%d*"}
